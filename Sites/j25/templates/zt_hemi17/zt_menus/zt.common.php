...
class MenuSystem
{
	...
	$xdelay = 350, $xduration = 350, $xtransition = 'Fx.Transitions.linear')
	{
		...
		$this->drill		= '<script type="text/javascript">var mymenu=new drilldownmenu({menuid: "drillmenu1", base: "'.JURI::base().'", menuheight: "auto", breadcrumbid: "drillcrumb", persist: {enable: true, overrideselectedul: true}})</script>';
	...
	*/
	...
	function showSubMegaMenu($row, $pid, $level, $cols)
	{
		...
		if(!$isgroup) {
			if(strpos($width,'px'))
				$width_wrap = "width:".((str_replace('px','',$width))+24)."px";
			else
				$width_wrap = "width:".($width+24)."px";
		} else {
			if(strpos($width,'px'))
