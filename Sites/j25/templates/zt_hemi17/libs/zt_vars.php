...
$ztcolorstyle = "ZTHemi17StyleCookieSite";

$get_ms 	= JRequest::getVar('menustyle');
$cookie_ms 	= $ztTools->get_cookie('ztmenustyle_hemi17');

$get_rtl 	= JRequest::getVar('direction');
$cookie_rtl = $ztTools->get_cookie('zt_rtl_hemi17');
...
if($get_ms){
	$menustyle = $get_ms;
	$ztTools->set_cookie('ztmenustyle_hemi17',$get_ms);
}
...
if($get_rtl && ($get_rtl == 'rtl' || $get_rtl == 'ltr') ) {
	$ztrtl = $get_rtl;
	$ztTools->set_cookie('zt_rtl_hemi17',$get_rtl);
}
...
if($ztTools->getParam('zt_mobile')) {
	...
	$cookieViewType = $ztTools->get_cookie('hemi17_ismobile');

	if($getViewType != NULL){
		$myBrowser = $getViewType;
		$ztTools->set_cookie('hemi17_ismobile', $myBrowser);
...
$modslide = $this->countModules('slideshow');
